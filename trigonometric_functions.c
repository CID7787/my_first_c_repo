...
double sine(double radian, error* err){// TODO safe_double_division_with_rounding
    if(!err){ return radian; }
    dbits radian_var = (dbits){.d = radian};
    dbits PI2_var = (dbits){.d = PI2};
    radian = my_fmod(radian_var, PI2_var, err);
...
double cosine(double radian, error* err){// TODO safe_double_division_with_rounding
    if(*err){ return radian; }
    dbits radian_var = (dbits){.d = radian};
    dbits PI2_var = (dbits){.d = PI2};
    radian = my_fmod(radian_var, PI2_var, err);
