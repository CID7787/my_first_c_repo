...
int8_t have_frac_part(dbits a){// TODO: fix function doesn't working correct
    int32_t norm_exp = a.parts.exp - (DOUBLE_EXP_BIAS & -(!!(a.parts.exp)) );
    return (norm_exp < 0) || ( a.parts.mantissa & ((DOUBLE_MANTISSA_IMPLICIT_ONE | MAX_DOUBLE_MANTISSA) >> else0(norm_exp > 0, norm_exp)) );
}